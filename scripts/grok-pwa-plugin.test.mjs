...
test("custom domain wins over VITE_PUBLIC_HOSTNAME grok.me for og:image", () => {
  assert.equal(
    resolvePublicHost("www.gypsyjazzhub.com", "glow-shale-falcon-daisy.grok.me"),
    "www.gypsyjazzhub.com",
  );
  assert.equal(
    resolvePublicHost("gypsyjazzhub.com", "glow-shale-falcon-daisy.grok.me"),
    "gypsyjazzhub.com",
  );
  assert.equal(
    resolvePublicHost("wild-race.grok.me", "glow-shale-falcon-daisy.grok.me"),
    "glow-shale-falcon-daisy.grok.me",
  );
  assert.equal(resolvePublicHost("app.vercel.app", "wild-race.grok.me"), "wild-race.grok.me");
});
...
test("custom-domain manifest uses site title, not Grok App", () => {
  const manifest = JSON.parse(
    renderWebManifest("www.gypsyjazzhub.com", { title: "Gypsy Jazz Hub" }),
  );
  assert.equal(manifest.name, "Gypsy Jazz Hub");
  assert.equal(manifest.short_name, "Gypsy Jazz Hub");
  assert.equal(manifest.start_url, "/");
  assert.equal(manifest.scope, "/");
