...
import { listConcerts, listLegendConcerts, type Concert } from "@/lib/api";
...
function isGypsyJazzBill(concert: Concert) {
  ...
  return /django|manouche|gypsy jazz|lautaresc|lăutăresc|hot club|sinti/.test(hay);
}

function atThisHouse(concert: Concert) {
  const hay = `${concert.venue} ${concert.city}`.toLowerCase();
  return /alhambra/.test(hay) && /bucharest|bucure/.test(hay);
}
...
export const Route = createFileRoute("/gradina-alhambra")({
  ...
  loader: async () => {
    ...
    const here = uniqueConcerts([...all, ...members.flat()]).filter(
